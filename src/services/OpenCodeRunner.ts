...
export class OpenCodeRunner {
  ...
  async runAgent(options: RunAgentOptions): Promise<RunAgentResult> {
    ...
    // OpenCode CLI: `opencode run "prompt"` for non-interactive execution
    const argsSpawn: string[] = ['run', prompt];
    if (sessionId) argsSpawn.push('--session', sessionId);

    return new Promise((resolve) => {
      const isWin = process.platform === 'win32';
      const command = isWin ? 'opencode.exe' : 'opencode';
      ...
      if (child.pid) {
        registerProcess(child.pid, { agentName: agentName || 'unknown', runner: 'opencode' }).catch(() => {});
      }
