...
async function init() {
  ...
  const architecture = await memorySearchTool({
    query: 'architecture projet standard overmind',
    limit: 3,
    include_runs: false,
  });
  ...
  console.error('\n--- Context Search ---');
  const context = await memorySearchTool({
    query: 'contexte projet contremaitre',
    limit: 3,
    include_runs: false,
  });
