...
export async function runOpenClawAgent(args: z.infer<typeof runOpenClawSchema>) {
  ...
  if (result.error?.includes('session') || result.error?.includes('EXIT_CODE_1')) {
    if (agentName) await deleteSessionId(agentName, finalConfig, 'openclaw');
  ...
  try {
    ...
    console.error(
      `[run_openclaw] ⚠️ Memory store failed: ${e instanceof Error ? e.message : String(e)}`,
    );
  }

  if (result.error)
    return {
      content: [{ type: 'text' as const, text: `❌ Erreur OpenClaw: ${result.error}` }],
      isError: true,
    };
  return {
    content: [
