...
export async function verifyInstallation(
  ...
  try {
    ...
    const checkCmd =
      process.platform === 'win32' ? `where ${meta.command}` : `which ${meta.command}`;
    ...
    if (meta.requiredVersion && meta.versionCmd) {
      try {
        ...
        if (
          !version.includes(meta.requiredVersion) &&
          !version.startsWith(meta.requiredVersion.split('.')[0])
        ) {
          return {
            ok: false,
            message:
              `⚠️ **Version Incorrecte pour ${meta.name}** : détectée ${version} (Attendu: ${meta.requiredVersion})\n\n` +
